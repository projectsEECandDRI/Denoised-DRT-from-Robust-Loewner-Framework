% Example_R_2RC_RL_ECM_RLF.m
%
% Extraction of the tempered Distribution of Relaxation Times (DRT) from 
% noisy impedance spectra using the Robust Loewner Framework (RLF), 
% applied to an R-2(RC)-RL Equivalent Circuit Model (ECM) containing two 
% RC elements and an RL element in series with ohmic resistance.
%
% This example script reproduces the results shown in Figure 5 and 
% Figure S4 of the paper:
%
...
plot(real(Hr_DRT_F_BIG),-imag(Hr_DRT_F_BIG),'-.',...
    'color',[0 0 1],'markersize',7,'linewidth',3,'DisplayName','RLF')
grid on; axis equal;
xlabel('Re(Z) [\Omega]','FontSize',18,'FontWeight','normal')
ylabel('-Im(Z) [\Omega]','FontSize',18,'FontWeight','normal')
xlim([-50 550]); ylim([-75 175]);
xticks([0 100 200 300 400 500]);
xticklabels({'0','100','200','300','400','500'});
yticks([-50 0 50 100 150]);yticklabels({'-50','0','50','100','150'});
legend('Location','NorthWest','FontSize',14,'FontWeight','normal','NumColumns',2)
...
grid on;
xlim([1e-18 1e3]); ylim([-100 300]);
xticks([1e-18 1e-15 1e-12 1e-9 1e-6 1e-3 1e0 1e3]);
xticklabels({'10^{-18}','10^{-15}','10^{-12}','10^{-9}','10^{-6}',...
    '10^{-3}','10^{0}','10^{3}'});
yticks([-100 0 100 200 300]);
yticklabels({'-100','0','100','200','300'});
